...
;WITH CTE_1 AS
(
SELECT Events.CHID, Events.EventID, DATEADD(MI, GMTOffset, Events.EventDateTime) AS EventDateTime, A.CP, A.TimeStampID, A.GroupID, B.CHID AS ClusterCHID
FROM Events
JOIN TimeStamps A ON Events.EventID = A.EventID
...
AND Events.EventDateTime  BETWEEN '2018-03-01 00:00:00.000' AND '2018-03-31 23:59:59.999' ---- 
), CTE_2 AS
(
...
BEGIN 
	...
	ELSE 
	BEGIN 
		IF CAST(@EventDateTime AS DATE) <>  CAST(@PreviousEventDateTime AS DATE)
		BEGIN
			SET @TrajectoryID += 1
			SET @SequenceID = 1
			SET @FirstEventDateTime = @EventDateTime
